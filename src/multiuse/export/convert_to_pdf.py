...
def rich_to_pdf(
    ...
    if highlighted_columns is None:
        highlighted_columns = [
            col for col in df.columns if col.endswith("_highlighted")
        ]
    ...
    display_order = ["ROW_INDEX_highlighted", *highlighted_columns]
...
def _rich_to_reportlab_inline(rich_text: str) -> str:
    ...
    for color_name, hex_color in text_colors.items():
        ...
        result = result.replace(
            f"&lt;&lt;&lt;FONT_END_{color_name}&gt;&gt;&gt;",
            "</font>",
        )

    return result.replace("&lt;&lt;&lt;CLOSE_TAG&gt;&gt;&gt;", "</span></font>")
