...
class ParquetIO:
    ...
    def read_parquet(
        ...
        columns_for_fake_df: list | None = None,
    ) -> pd.DataFrame:
    ...
    def _validate_fpath_after_write(self, fpath: Path) -> None:
        ...
        if not fpath.exists():
            fe_string = f"Failed to write to {fpath=}"
            raise FileNotFoundError(fe_string)
    ...
    for col in df.columns:
        if hasattr(df[col], "str"):
            ...
            if df[col].str.contains(r"{").any():
                df[col] = df[col].apply(load_from_json)
        elif df[col].dtype == "object":
            df[col] = df[col].astype(str).apply(load_from_json)
